...
vi.mock('@graph/state.js', () => ({ state: mockState }));

// Mock lod to avoid its state dependency
vi.mock('@graph/lod/lod.js', () => ({
    selectLevel: () => 0,
}));

// Mock viewport
vi.mock('@graph/render/viewport.js', () => ({
    getViewport: () => ({ minX: -500, maxX: 500, minY: -300, maxY: 300 }),
}));

// Mock simplify-force
vi.mock('@graph/data/simplify-force.js', () => ({
    getForceNodes: () => [],
}));
...
} = await import('@graph/utils/hit-test.js');
